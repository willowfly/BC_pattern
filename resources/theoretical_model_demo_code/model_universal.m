...
function gui()
    ...
    h.main.Name = 'Cross-talk model';
    h.main.MenuBar = 'none';
    h.main.ToolBar = 'none';
    h.main.NumberTitle = 'off';
    % create the control objects
    params = {'HLL','HRR','HLR','HRL'};
...
function createControl(i,N,string)
    ...
    h.panel(i).amp = uicontrol(h.panel(i).panel,'style','slider',...
        ...
        'Max',0,'Min',-40,'SliderStep',1/40*[1,1],...
...
function calculate_and_update()
    global h p
    HLL = p.para(1); HRR = p.para(2);
    HLR = p.para(3); HRL = p.para(4);
    ...
    for jj = 1:length(p.IPD)
        ...
        Dc(ii,jj) = (HLR+Ds*HRR)/(HLL+Ds*HRL); 
    end
    end
